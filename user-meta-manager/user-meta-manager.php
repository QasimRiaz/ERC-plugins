...
';    
    ...
    $output .= '>'.__('File Upload', UMM_SLUG).'</option>
    <option value="date"';
...
function umm_show_profile_frontend_fields($echo=true, $fields=false, $mode='profile', $form_id=false, $debug=false){
   ...
   $umm_settings = $umm_options['settings'];
    ...
    $html_before =  '<h3 class="umm-custom-fields">[section-title]</h3>
<table class="mytable table table-striped table-bordered table-condensed" ><thead><tr class="text_th"><th>Due Date</th><th>Task</th><th>Specifications</th><th>Action</th><th>Status</th></tr></thead><tbody>';
  ...
  //  print_r($pieces);
    ...
    if((!$show_fields && $profile_field_settings['add_to_profile'] == 'yes' && $user_can_view) || ($show_fields && array_key_exists($profile_field_name, $umm_data) && $user_can_view)):
      ...
      switch($profile_field_settings['type']){
            ...
            case 'url':
            case 'week':           
            $field_html .= '<input style="width:212px;" type="' . $profile_field_settings['type'] . '" name="' . $profile_field_name;
            ...
            $field_html .= '<input class="uploadFileid"  id="display_my'.$profile_field_name.'" placeholder="Choose File" disabled="disabled" /><div class="fileUpload btn btn-primary"><span>Browse</span><input type="file" class ="upload myfileuploader" id="my'.$profile_field_name.'" name="my'.$profile_field_name.'" /></div>';
             if(!empty($value)){
            $field_html .='</div>';    
            }
            $field_html .= '<input type="hidden" name="' . $profile_field_name;
            if($mode == 'adduser') $field_html .= '[]';
            $field_html .= '" value="' . $value . '" class="' . stripslashes(htmlspecialchars_decode($profile_field_settings['class'])) . $unique . '"';
            if($profile_field_settings['required'] == 'yes')
            $field_html .= ' required="required"';
            ...
            $field_html .= $form_tag . " />";
             if(!empty($value)){
                 $field_html .= "<div class='remove_".$profile_field_name."'><a href='".$base_url."/wp-admin/users.php?page=my-sponsor-reports&userid=".$user_IDD."&fieldname=".$profile_field_name."' target='_blank' style='margin-right: 77px;'>Download File</a><a  style='width:49px;' id='remove_".$profile_field_name."' class='". $profile_field_name." btn-danger btn remove_upload' >Remove</a></div>"; 
             }
            break;  
            case 'textarea':
            $field_html .= '<textarea style="width:212px;" name="' . $profile_field_name;
            if($mode == 'adduser') $field_html .= '[]';
          ...
          //Modification by Qasim Riaz
            case 'link':
            $field_html .= '<a href="'.$profile_field_settings['lin_url'].'" target="_blank">'.$profile_field_settings['linkname'].'</a>';
            break;
            ...
            $field_html .= '<select name="' . $profile_field_name . $multi . '" onchange="update_time_stamp(this)" id="' . $profile_field_name . $multi . '" class="selectclass ' . stripslashes(htmlspecialchars_decode($profile_field_settings['class'])) . $unique . '"';
            if($profile_field_settings['required'] == 'yes')
            $field_html .= ' required="required"';
            if(!empty($profile_field_settings['attrs']))
...
$task_value=substr($taskid[0],1);
     ...
     if(strpos($profile_field_name, "status") == false){
         ...
         $attribute = '<tr class="overdue"><td class="duedate">'.$profile_field_settings['attrs'].'  <i class="icon-flag"></i></td><td class="checklist">'.$profile_field_settings['label'].'</td><td class="descrpition">'.$profile_field_settings['descrpition'].'</td>';
   
           
         }else{
              $attribute = '<td class="duedate">'.$profile_field_settings['attrs'].'</td><td class="checklist">'.$profile_field_settings['label'].'</td><td class="descrpition">'.$profile_field_settings['descrpition'].'</td>';
   ...
   <tbody>' : stripslashes(htmlspecialchars_decode($umm_settings['html_before_' . $mode]));
    ...
    if((!$show_fields && $profile_field_settings['add_to_profile'] == 'yes' && $user_can_view) || ($show_fields && array_key_exists($profile_field_name, $umm_data) && $user_can_view)):
      ...
      switch($profile_field_settings['type']){
            ...
            $field_html .= $form_tag . " />";
             if(!empty($value)){
                 $field_html .= "<a href='".$base_url."/wp-admin/users.php?page=my-sponsor-reports&userid=".$user_IDD."&fieldname=".$profile_field_name."' target='_blank'>Download File</a>"; 
             }else{ $field_html .= "No File"; }
            break;  
            case 'textarea':
...
//for ($x=1; $x<=count($_FILES); $x++) {
  
//}
        
   // echo $values = get_user_meta(1, 'task_2_action',true);
      //Modification by Qasim Riaz
       $umm_data = umm_get_option('custom_meta');
       $umm_settings = umm_get_option('settings');
       $profile_fields = umm_get_option('profile_fields');
       ...
       $content = '<form id="' . $form_id . '" action="#" method="post" class="' . $class .  '" enctype="multipart/form-data">
  ...
  /*      
 foreach ($_FILES["myuploadedfiles"]["error"] as $key => $error) {
        ...
        foreach ($files['name'] as $key => $value) 
    {
         $file = array(
           ...
           // echo "File is valid, and was successfully uploaded.\n";
            //var_dump( $movefile);
            echo   $fileurl = $movefile['url'];
           
           //update_user_meta(1, 'task_2_action', $fileurl);
         //  $user_id =$current_user->ID;
         //  $task_id='task_'.$u.'_action';
         //  update_user_meta($user_id, $task_id , $fileurl);
         } else {
...
//print_r($_FILES);
 ...
 for ($u=0; $u<=substr_count($_POST['umm_fields'], 'task'); $u++) {
     
     $taskvalues=  explode(',', $_POST['umm_fields']);
     //echo $taskvalues[$u].'--------'.$_FILES["my".$taskvalues[$u]]['name'].'</br>';
     //echo $_FILES["my".$taskvalues[$u]]['name'];
     
  if(!empty($_FILES["my".$taskvalues[$u]]['name'])) {
       //echo $taskvalues[$u].'--------';
    if ( ! function_exists( 'wp_handle_upload' ) ) require_once( ABSPATH . 'wp-admin/includes/file.php' );
         $uploadedfile = $_FILES["my".$taskvalues[$u]];
         $upload_overrides = array( 'test_form' => false, 'mimes' => array('ai' => 'application/postscript','jpg|jpeg|jpe' => 'image/jpeg','gif' => 'image/gif','png' => 'image/png','bmp' => 'image/bmp','pdf'=>'text/pdf','doc'=>'application/msword','docx'=>'application/msword','xlsx'=>'application/msexcel') );
  ...
  //Modification by Qasim Riaz
$user_id =$current_user->ID;
if ( function_exists('my_profile_update_sf')){
my_profile_update_sf($user_id);
}
$content .= '<div class="alert wpb_content_element alert-success"><div class="messagebox_text"><p>' . $success . '</p></div></div>' . "\n";
    ...
    */
    ...
    $content .= umm_show_profile_frontend_fields(false, $atts['fields'], 'shortcode', $form_id) . '
    <div class="mysubmit"><button type="submit" class="wpb_button btn-success btn-large" form="' . $form_id . '">' . $submit .  '</button></div>' . "\n";
